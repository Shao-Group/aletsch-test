...
process_dataset() {
    ...
    do
        rm -rf $cur/cov_$cov
        mkdir -p $cur/cov_$cov

        vdscript=$dataset-$aligner-$cov.sh
        echo "cd $cur" > $vdscript
        echo "{ /usr/bin/time -v $psiclass --lb $list --stranded $strand -o $cur/cov_$cov/ --vd $cov -p 10 > $cur/cov_$cov/psiclass.log ; } 2> $cur/cov_$cov/psiclass.time" >> $vdscript
        echo "mv $cur/cov_$cov/psiclass_vote.gtf $cur/psiclass_$cov.gtf" >> $vdscript

        stats=$cur/$cov.mul.stats
        echo "$gffcompare -M -N -r $ref -o $stats psiclass_$cov.gtf" >> $vdscript
        #echo "refnum=\$(cat $stats | grep Reference | grep mRNA | head -n 1 | awk '{print \$5}')" >> $script
        #echo "$gtfcuff roc $cov.mul.psiclass_$cov.gtf.tmap \$refnum cov > cov_$cov.roc.mul" >> $script
        echo "less $stats | awk '{print \$4}' | sed -n '18p' >> psiclass.mat" >> $vdscript
        echo "less $stats | awk '{print \$6}' | sed -n '14p' >> psiclass.pre" >> $vdscript

        if [ "$cov" = "1" ]; then
            echo "cp $cur/cov_1/psiclass_sample_*.gtf $sgtf/" >> $vdscript
            echo "cd $sgtf" >> $vdscript
            echo "cat gff-scripts | xargs -L 1 -I CMD -P 10 bash -c CMD 1> /dev/null 2> /dev/null &" >> $vdscript
            echo "cd $cur" >> $vdscript
        fi

        chmod u+x $vdscript
        nohup bash $vdscript > "${cur}/nohup.$cov.log" 2>&1 &
