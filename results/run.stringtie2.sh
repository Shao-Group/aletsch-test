...
process_dataset() {
    ...
    k=0
    rm -f $cur/st.gtf.list
    while IFS= read -r bam_file; do
        kscript=$sgtf/$k.sh
        echo "cd $sgtf" > $kscript
        echo "{ /usr/bin/time -v $stringtie2 $bam_file -c 0.001 -p 10 -o $k.stringtie2.gtf $strand > $k.stringtie2.log; } 2> $k.stringtie2.time" >> $kscript
        echo "$gffcompare -M -N -r $ref -o $k.mul.stats $k.stringtie2.gtf" >> $kscript
        echo "refnum=\$(cat $k.mul.stats | grep Reference | grep mRNA | head -n 1 | awk '{print \$5}')" >> $kscript
        echo "$gtfcuff roc $k.mul.$k.stringtie2.gtf.tmap \$refnum cov > $k.roc.mul" >> $kscript
        echo "$kscript" >> $sgtf/gff-scripts
        chmod u+x $kscript

        echo "$sgtf/$k.stringtie2.gtf" >> $cur/st.gtf.list    
        ((k++))
    done < "$list"
    ...
    echo "Submit job: StringTie2 for $dataset"
}
