#!/bin/bash
source ~/.profile
export LD_LIBRARY_PATH=/data/shared/tools/boost/boost_1_80_0/lib:$LD_LIBRARY_PATH
covlist='0 0.5 1 2 5 10 20 30 50 80 100 150'
covlist2='200 250 300 400 500'
...
results=$dir/transmeta
aligner="star"

# Check for required tools
missing_tools=()
[ ! -f "$transmeta" ] && missing_tools+=("transmeta")
...
process_dataset() {
    ...
    echo "{ /usr/bin/time -v $transmeta -B $list -s $strand -o $cur/tmp -p 10 > $cur/transmeta.log ; } 2> $cur/transmeta.time" > $script
    echo "mv $cur/tmp/TransMeta.gtf $cur/TransMeta-2.gtf" >> $script
    echo "mv $cur/tmp/TransMeta-*.gtf $cur/" >> $script
    
    for cov in $covlist;
    do
        stats=$cur/$cov.mul.stats
        echo "$gffcompare -M -N -r $ref -o $stats $cur/TransMeta-$cov.gtf" >> $script
        echo "less $stats | awk '{print \$4}' | sed -n '18p' >> transmeta.mat" >> $script
        echo "less $stats | awk '{print \$6}' | sed -n '14p' >> transmeta.pre" >> $script
    done

    for cov2 in $covlist2
    do
        echo "{ /usr/bin/time -v $transmeta -B $list -s $strand -o $cur/tmp_$cov2 -c $cov2 -p 10 > $cur/transmeta_$cov2.log ; } 2> $cur/transmeta_$cov2.time" >> $script
        echo "cp $cur/tmp_$cov2/TransMeta.gtf $cur/TransMeta-$cov2.gtf" >> $script
        stats=$cur/$cov2.mul.stats
        echo "$gffcompare -M -N -r $ref -o $stats $cur/TransMeta-$cov2.gtf" >> $script
        echo "less $stats | awk '{print \$4}' | sed -n '18p' >> $cur/transmeta.mat" >> $script
        echo "less $stats | awk '{print \$6}' | sed -n '14p' >> $cur/transmeta.pre" >> $script
    done
    ...
    ln -sf $gtfcuff $sgtf
    for k in $(seq 1 $maxid)
    do
        kscript=$sgtf/$k.sh
        echo "./gffcompare -M -N -r $(basename $ref) -o $k.mul.stats TransMeta.bam$k.gtf" > $kscript
        echo "$kscript" >> $sgtf/gff-scripts
        chmod u+x $kscript
    done

    echo "mv tmp/TransMeta.bam* gtf/" >> $script
    echo "cd $sgtf" >> $script
    echo "cat gff-scripts | xargs -L 1 -I CMD -P 10 bash -c CMD 1> /dev/null 2> /dev/null &" >> $script

    chmod +x $script
    nohup bash $script > "${cur}/nohup.log" 2>&1 &
    echo "Submit job: Transmeta for $dataset"
}
