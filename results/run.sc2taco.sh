...
process_dataset() {
    ...
    #find $cur -maxdepth 1 -type f ! -name "scallop2.gtf.list" -exec rm -f {} +
    
    list="$cur/scallop2.gtf.list"
    #mv $list "$cur/scallop2.gtf.full.list"

    find "$cur" -name "*.scallop2.gtf" > $list
    ...
    do
        ...
        rm -rf $cur/$cov
        echo "{ /usr/bin/time -v $taco $list -o $cur/$cov --gtf-expr-attr cov --filter-min-expr $cov -p 10 ; } 2> sc2taco-$cov.time" >> $script
        echo "$gffcompare -M -N -r $ref -o $stats $cur/$cov/assembly.gtf" >> $script
        echo "less $stats | awk '{print \$4}' | sed -n '18p' >> sc2taco.mat" >> $script
        echo "less $stats | awk '{print \$6}' | sed -n '14p' >> sc2taco.pre" >> $script
    done    
    chmod +x $script

    nohup bash $script > "${cur}/nohup.log" 2>&1 &
    echo "Submit job: Scallop2-Taco for $dataset"
}
